...
<AccordionGroup>
  ...
  <Accordion title="Populate incident custom fields" icon="file">
  ...
  <Accordion title="Trigger workflows" icon="workflow">
  ...
  <Accordion title="Learn from insights" icon="chart-line">
  ...
  <Accordion title="Send to the right channels" icon="message">
...
<Steps>
  ...
  <Step title="Access Attributes">
    Click the **Settings icon** (gear icon in top right) and select **Attributes**
  </Step>
  <Step title="Add new attribute">
    Click **+ Add attribute** to create a new attribute (see [recommended attributes](#recommended-attributes) below)
  </Step>
  ...
  <Step title="Save changes">
    Click **Save changes** to create the attribute
  </Step>
</Steps>
...
<Steps>
  <Step title="Complete alert source setup">
    Follow the [alert source setup](/product-documentation/on-call/alert-sources) steps and navigate to the final stage **Configure your setup**
  </Step>
  <Step title="Review AI suggestions">
    Click **Review attributes** to see AI suggestions based on your payload
  </Step>
  <Step title="Accept or skip suggestions">
  ...
  <Step title="Verify extracted attributes">
...
<Tabs>
  <Tab title="Link existing attribute">
  ...
  <Tab title="Create custom extraction">
...
<CodeGroup>
```javascript Extract from nested arrays
...
```javascript Add fallback values
// Try multiple possible locations for team name
$.metadata.team || $.labels.team || "unknown"
```
</CodeGroup>

The fallback example tries `$.metadata.team` first, then `$.labels.team` if that's empty, and finally defaults to "unknown" if neither exists.
